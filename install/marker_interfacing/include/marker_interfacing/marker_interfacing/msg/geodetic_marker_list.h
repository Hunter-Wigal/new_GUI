// generated from rosidl_generator_c/resource/idl.h.em
// with input from marker_interfacing:msg/GeodeticMarkerList.idl
// generated code does not contain a copyright notice

#ifndef MARKER_INTERFACING__MSG__GEODETIC_MARKER_LIST_H_
#define MARKER_INTERFACING__MSG__GEODETIC_MARKER_LIST_H_

#include "marker_interfacing/msg/detail/geodetic_marker_list__struct.h"
#include "marker_interfacing/msg/detail/geodetic_marker_list__functions.h"
#include "marker_interfacing/msg/detail/geodetic_marker_list__type_support.h"

#endif  // MARKER_INTERFACING__MSG__GEODETIC_MARKER_LIST_H_
